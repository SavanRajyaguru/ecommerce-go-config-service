...
WORKDIR /build/ecommerce-go-config-service
...
COPY --from=builder /build/ecommerce-go-config-service/main .

# Copy config files if they exist
COPY --from=builder /build/ecommerce-go-config-service/config* ./config/ 2>/dev/null || true
